...
> 安裝完直接問 6 個短問題；認識你後，立刻陪你完成第一件小事。
...
這套 AI 陪跑教練不是單純教你「怎麼問 AI」。請 Codex 或 Claude Code 安裝完後，它會在同一個對話直接用 6 個短問題認識你，再帶你完成第一件小事。之後開新對話時，用 `vv` 開頭再說出任務就好。
...
6. `onboarding.md`：安裝完後，vv 在同一個對話用 6 題短問答建立第一版記憶。
...
```text
請先讀 ~/指揮家.md 和 onboarding.md，進入 AI 陪跑教練模式。現在直接問第 1 題，一次只問一題；第 6 題答完後，再開始我選的第一件小事。
```

## 第一次要做什麼

如果是 Codex 或 Claude Code 在目前對話幫你安裝，它會在檢查完檔案後直接打開 `onboarding.md`，一題一題問 6 題。你不用關掉對話，也不用再輸入任何啟動詞。
...
不需要一次寫很完美。你回答完一題，vv 再問下一題；第 6 題就是第一件小事，答完後立刻開工。
...
如果你是第一次安裝、還沒建立 Vault，AI 陪跑教練會在安裝完後直接自我介紹，再用 6 題建立第一版 Vault。第 6 題會接上你的第一個小任務；這一次不用再打 `vv`。
