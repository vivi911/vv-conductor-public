...
但你可以請 Codex 幫你檢查 GitHub 上有沒有新版：

```text
vv 檢查更新
```

Codex 會比對本機安裝版和 GitHub 公開包：

- GitHub 公開包：`https://github.com/vivi911/vv-conductor-public`
- 本機 Codex skill：`~/.codex/skills/vv-conductor/`
- 版本檔：repo 根目錄的 `VERSION`，以及安裝後的 `~/.codex/skills/vv-conductor/VERSION`

如果 GitHub 有新版，就要重新下載 repo，並覆蓋本機 skill。只看 GitHub 有更新還不夠，因為 Codex 真正讀的是你電腦裡的 `~/.codex/skills/vv-conductor/`。
...
如果記憶有接上，它會接著最新進度跟你聊。如果記憶讀不到，它會明講「我現在讀不到記憶」，不會假裝知道。
