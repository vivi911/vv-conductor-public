...
你可以叫它 `vv`。**vv 就是你自己專屬的 AI 教練**——不是只有第一次才能用，以後每次打開新對話，打 `vv` 就能把他叫出來。他會先看你是誰、現在有哪些事在跑，再判斷這件事可以自動做，還是要先停下問你。（`vv vault` 會特別提醒它先把你的記憶庫讀進來再做事。）
...
每次打開新對話，先打一個「觸發詞」叫醒 vv，再講你要做的事就好——**不限第一次，這是你以後每天都可以用的固定叫法**：

| 你打什麼 | vv 會做什麼 | 什麼時候用 |
|---|---|---|
| `hi` | 第一次先問你想做什麼並安全開工；有 Vault 時接回上次進度 | 第一次開始、每天開工 |
| `vv` | 叫出你的 AI 教練，開始判斷任務、派工、把關 | 任何時候都可以，要它幫你做一件事就打這個 |
