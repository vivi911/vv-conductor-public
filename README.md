...
這套 AI 陪跑教練不是單純教你「怎麼問 AI」。第一次只要打招呼，它會用幾個短問題認識你，並用白話帶你完成第一件小事；如果你一開口已經有任務，它就直接接住，不把你拉去填問卷。
...
6. `onboarding.md`：第一次打招呼時，vv 用 7 題短問答建立第一版記憶。

## 平常對話怎麼開始

每天開一個新對話，直接講你要做的事就好。不用先選 Codex 或 Claude Code，也不用背 `hi`、`vv` 這類觸發詞。
...
一句話：**開對話 → 用白話講你要做的事**。不用背指令、不用寫複雜提示詞。
...
v1.5 是「AI 做事前的安全規則」。
v1.7.1 是「不用選平台或背觸發詞；先用幾個短問題認識你，再帶你完成第一件小事」。
...
安裝完成後，關掉再重開你要用的 AI 工具。聊天視窗必須等你送出第一句話才能回應，這是所有聊天工具的限制；但你的第一句不再是咒語，直接用白話說需求即可，例如：
...
安裝後開一個新對話，直接用白話說你想做的事，例如：「我想把客戶會議紀錄整理成待辦事項。」AI 陪跑教練會先問：「你現在最想請 AI 幫你做什麼？」接著幫你講清楚風險、縮成安全的第一版，不會先丟 7 題考卷。
