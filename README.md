...
你可以叫它 `vv`。**vv 就是你自己專屬的 AI 教練**——不是只有第一次才能用，以後每次打開新對話，打 `vv` 就能把他叫出來。他會先看你是誰、現在有哪些事在跑，再判斷這件事可以自動做，還是要先停下問你。
...
它也有開機規則。以後你打開新對話，只要打 `vv`，它就會先抓最新事件和進度接著聊，不用每次重新交代「我是誰、上次做到哪、這個案子卡在哪」。
...
## 平常對話怎麼用 vv（只要記得一個字：`vv`）

每次打開新對話，打 `vv` 就好——**不用另外記「第一次要打什麼」「想確認記憶要打什麼」，一個字就夠**。只要你已經建過 Vault，vv 會自動先讀記憶，回你上次做到哪、下一步是什麼，手上還有哪些專案沒完成，不用你另外問。
...
`hi`、`指揮家`、`AI 陪跑教練`、`vv vault` 這幾個字也都能叫醒他、效果完全一樣——只是備用說法，你不需要特地記，打 `vv` 就好。
...
一句話：**開對話 → 打 `vv` → 講你要做的事**。不用背指令、不用寫複雜提示詞。
...
裝完**重開** Codex 或 Claude Code，當你說 `hi`、`AI 陪跑教練`、`開工手冊`、`vv`、`指揮家`、`今天先做什麼`、`派工`、`紅黃綠` 或 `handoff`，它就會觸發這個 skill。
