...
一般 AI 像失憶症，每次開新對話都要你重講一遍自己是誰；Vault 就是讓 AI「記得你」的那個地方。Vault 是進階的長期陪跑功能，不是第一次開工前的考卷。建立後，它會先讀 Vault，再接著上次的進度繼續。
...
v1.5 是「AI 做事前的安全規則」。
v1.7.0 是「不用選平台或背觸發詞，先安全完成第一件事，再決定要不要讓 AI 長期認識你」。
