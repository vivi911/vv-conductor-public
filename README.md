...
下載這包之後，你會得到一套讓 AI 更像工作教練的規則。

以後你在 Codex 裡打 `vv`，它就會啟動一整套管理 AI 的方法：先看你是誰、現在有哪些事在跑，再判斷這件事能不能自動做，還是要先停下問你。

它有全域記憶，會記得你是誰、專案做到哪、哪些事不能亂碰。也有開機規則，以後你打 `hi`，它會先抓最新進度接著聊，不用每次重新交代背景。

裡面也內建一組開發角色 agent：PM、架構師、UX、開發、測試、發布。你不用一開始就講得很完整，vv 會幫你把模糊想法一步一步理順成能動工的計畫。
